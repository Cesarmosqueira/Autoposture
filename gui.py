...
def create_pose_estimation_gui():
    root = tk.Tk()
    root.title('Pose estimation App')

    def run_pose_estimation():
        opt = parse_opt()
        print(opt)
        strip_optimizer(opt.device, opt.poseweights)
        run(poseweights = "src_models/yolov7-w6-pose.pt", source="0", device="0", view_img=True, save_conf=False, line_thickness=3, hide_labels=True, hide_conf=False)

    start_button = ttk.Button(root, text="Start Pose Estimation", command=run_pose_estimation)
    start_button.pack()

    root.mainloop()

if __name__ == "__main__":
    create_pose_estimation_gui()
